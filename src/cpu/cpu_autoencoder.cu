...
Cpu_Autoencoder::Cpu_Autoencoder(): IAutoencoder() {};

Cpu_Autoencoder::Cpu_Autoencoder(const char *filename): IAutoencoder(filename) {};
...
Dataset Cpu_Autoencoder::_encode_save_output(const Dataset &dataset) {
    ...
    // Dim: n * w/2 * w/2 * 128
    cpu_add_bias(_out_encoder_filter_2.get(), _encoder_bias_1.get(), _out_encoder_bias_2.get(),
    ...
    memcpy(res.get_data(), _out_max_pooling_2.get(), n * width / 4 * height / 4 * _ENCODER_FILTER_2_DEPTH * sizeof(float));
...
Dataset Cpu_Autoencoder::_decode_save_output(const Dataset &dataset) {
    ...
    memcpy(res.get_data(), _out_decoder_bias_3.get(),
            n * 4 * width * 4 * height * _DECODER_FILTER_3_DEPTH * sizeof(float));
...
float Cpu_Autoencoder::_fit_batch(const Dataset &batch, float learning_rate) {
    ...
    // Update bias
    cpu_bias_grad(d_out, d_in,
                    n, width , height, _DECODER_FILTER_3_DEPTH);

    cpu_update_weight(_decoder_bias_3.get(), d_in, _DECODER_FILTER_3_DEPTH, learning_rate);

    // Update filter
    cpu_conv2D_grad(_out_upsampling_2.get(), d_out, d_filter,
                    n, width, height, _DECODER_FILTER_2_DEPTH, _DECODER_FILTER_3_DEPTH);

    // Pass delta backwards
    cpu_conv2D(d_out, _decoder_filter_3.get(), d_in,
                n, width, height, _DECODER_FILTER_2_DEPTH, _DECODER_FILTER_3_DEPTH);
    ...
    // Update weight
    cpu_update_weight(_decoder_filter_3.get(), d_filter, _DECODER_FILTER_3_SIZE, learning_rate);

    // Pass through upsampling (dim: n * w/2 * w/2 * 256)
    cpu_upsampling_backward(d_out, d_in,
                            n, width / 2, height / 2, _DECODER_FILTER_2_DEPTH);

    // Pass through ReLU (d_in and d_out swapped)
    cpu_relu_backward(_out_decoder_bias_2.get(), d_in, d_out,
                        n, width / 2, height / 2, _DECODER_FILTER_2_DEPTH);
    ...
    cpu_update_weight(_decoder_bias_2.get(), d_in, _DECODER_FILTER_2_DEPTH, learning_rate);

    cpu_conv2D_grad(_out_upsampling_1.get(), d_out, d_filter,
                    n, width / 2, height / 2, _DECODER_FILTER_1_DEPTH, _DECODER_FILTER_2_DEPTH);

    cpu_conv2D(d_out, _decoder_filter_2.get(), d_in,
                n, width / 2, height / 2, _DECODER_FILTER_1_DEPTH, _DECODER_FILTER_2_DEPTH);

    swap(d_out, d_in);
    cpu_update_weight(_decoder_filter_2.get(), d_filter, _DECODER_FILTER_2_SIZE, learning_rate);

    // Upsampling (dim: n * w/4 * w/4 * 128)
    cpu_upsampling_backward(d_out, d_in,
                            n, width / 4, height / 4, _DECODER_FILTER_1_DEPTH);

    // ReLU
    cpu_relu_backward(_out_decoder_bias_1.get(), d_in, d_out,
                        n, width / 4, height / 4, _DECODER_FILTER_1_DEPTH);
    ...
    cpu_update_weight(_decoder_bias_1.get(), d_in, _DECODER_FILTER_1_DEPTH, learning_rate);

    cpu_conv2D_grad(_out_max_pooling_2.get(), d_out, d_filter,
                    n, width / 4, height / 4, _ENCODER_FILTER_2_DEPTH, _DECODER_FILTER_1_DEPTH);

    cpu_conv2D(d_out, _decoder_filter_1.get(), d_in,
                n, width / 4, height / 4, _ENCODER_FILTER_2_DEPTH, _DECODER_FILTER_1_DEPTH);

    swap(d_out, d_in);
    cpu_update_weight(_decoder_filter_1.get(), d_filter, _DECODER_FILTER_1_SIZE, learning_rate);

    // Max pooling backwards (dim: n * w/2 * w/2 * 128)
    cpu_max_pooling_backward(_out_encoder_relu_2.get(), d_out, d_in,
                            n, width / 2, height / 2, _ENCODER_FILTER_2_DEPTH);

    cpu_relu_backward(_out_encoder_bias_2.get(), d_in, d_out,
                        n, width / 2, height / 2, _ENCODER_FILTER_2_DEPTH);
    ...
    cpu_update_weight(_encoder_bias_2.get(), d_in, _ENCODER_FILTER_2_DEPTH, learning_rate);
        
    cpu_conv2D_grad(_out_max_pooling_1.get(), d_out, d_filter,
                    n, width / 2, height / 2, _ENCODER_FILTER_1_DEPTH, _ENCODER_FILTER_2_DEPTH);

    cpu_conv2D(d_out, _encoder_filter_2.get(), d_in,
                n, width / 2, height / 2, _ENCODER_FILTER_1_DEPTH, _ENCODER_FILTER_2_DEPTH);

    swap(d_out, d_in);
    cpu_update_weight(_encoder_filter_2.get(), d_filter, _ENCODER_FILTER_2_SIZE, learning_rate);

    cpu_max_pooling_backward(_out_encoder_relu_1.get(), d_out, d_in,
                                n, width, height, _ENCODER_FILTER_1_DEPTH);

    cpu_relu_backward(_out_encoder_bias_1.get(), d_in, d_out,
                        n, width, height, _ENCODER_FILTER_1_DEPTH);
    ...
    cpu_update_weight(_encoder_bias_1.get(), d_in, _ENCODER_FILTER_1_DEPTH, learning_rate);

    cpu_conv2D_grad(batch.get_data(), d_out, d_filter,
                    n, width, height, depth, _ENCODER_FILTER_1_DEPTH);

    cpu_conv2D(d_out, _encoder_filter_1.get(), d_in,
                n, width, height, depth, _ENCODER_FILTER_1_DEPTH);
...
Dataset Cpu_Autoencoder::encode(const Dataset &dataset) const {
    ...
    int encoded_image_bytes = width / 4 * height / 4 * _ENCODER_FILTER_2_DEPTH * sizeof(float);
    int offset  = 0;
    ...
    unique_ptr<float[]> a = make_unique<float[]>(_ENCODE_BATCH_SIZE * width * height * MAX_FILTER_DEPTH);
    unique_ptr<float[]> b = make_unique<float[]>(_ENCODE_BATCH_SIZE * width * height * MAX_FILTER_DEPTH);

    for (int i = 0; i < batches.size(); ++i) {
        int n = batches[i].n;

        // First conv2D
        cpu_conv2D(batches[i].get_data(), _encoder_filter_1.get(), a.get(),
                n, width, height, depth, _ENCODER_FILTER_1_DEPTH);

        // Add bias
        cpu_add_bias(a.get(), _encoder_bias_1.get(), b.get(),
                    n, width, height, _ENCODER_FILTER_1_DEPTH);

        // ReLU
        cpu_relu(b.get(), a.get(),
                n, width, height, _ENCODER_FILTER_1_DEPTH);

        // Max pooling
        cpu_max_pooling(a.get(), b.get(),
                        n, width, height, _ENCODER_FILTER_1_DEPTH);

        // Second conv2D
        cpu_conv2D(b.get(), _encoder_filter_2.get(), a.get(),
                n, width / 2, height / 2, _ENCODER_FILTER_1_DEPTH, _ENCODER_FILTER_2_DEPTH);

        cpu_add_bias(a.get(), _encoder_bias_2.get(), b.get(),
                    n, width / 2, height / 2, _ENCODER_FILTER_2_DEPTH);
...
Dataset Cpu_Autoencoder::decode(const Dataset &dataset) const {
    int width = dataset.width, height = dataset.height, depth = dataset.depth;
    int encoded_image_bytes = 4 * width * 4 * height * _DECODER_FILTER_3_DEPTH * sizeof(float);
    int offset  = 0;

    vector<Dataset> batches = create_minibatches(dataset, _ENCODE_BATCH_SIZE);
    Dataset res(dataset.n, width * 4, height * 4, _DECODER_FILTER_3_DEPTH);
    ...
    for (int i = 0; i < batches.size(); ++i) {
        ...
        // First conv2D
        cpu_conv2D(batches[i].get_data(), _decoder_filter_1.get(), a.get(),
                    n, width, height, depth, _DECODER_FILTER_1_DEPTH);

        // Add bias
        cpu_add_bias(a.get(), _decoder_bias_1.get(), b.get(),
                    n, width, height, _DECODER_FILTER_1_DEPTH);
        ...
        // Upsampling
        cpu_upsampling(a.get(), b.get(),
                    n, width, height, _DECODER_FILTER_1_DEPTH);

        // Second conv2D
        cpu_conv2D(b.get(), _decoder_filter_2.get(), a.get(),
                    n, width * 2, height * 2, _DECODER_FILTER_1_DEPTH, _DECODER_FILTER_2_DEPTH);

        cpu_add_bias(a.get(), _decoder_bias_2.get(), b.get(),
                    n, width * 2, height * 2, _DECODER_FILTER_2_DEPTH);
        ...
        // Third conv2D
        cpu_conv2D(b.get(), _decoder_filter_3.get(), a.get(),
                    n, width * 4, height * 4, _DECODER_FILTER_2_DEPTH, _DECODER_FILTER_3_DEPTH);

        cpu_add_bias(a.get(), _decoder_bias_3.get(), b.get(),
                    n, width * 4, height * 4, _DECODER_FILTER_3_DEPTH);
...
float Cpu_Autoencoder::eval(const Dataset &dataset) {
    return cpu_mse_loss(dataset.get_data(), decode(encode(dataset)).get_data(),
