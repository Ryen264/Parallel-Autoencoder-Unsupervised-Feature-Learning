    ...
    for (int i = 0; i < height / 2; ++i) {
      for (int j = 0; j < width / 2; ++j) {
        ...
        int neighbors_idx[] = { image * GET_1D_INDEX(i * 2, j * 2, 0, width, depth),
                                ...
                                image * GET_1D_INDEX(i * 2 + 1, j * 2 + 1, 0, width, depth) };
  ...
  for (int image = 0; image < n; ++image) {
    for (int i = 0; i < height; ++i) {
      for (int j = 0; j < width; ++j) {
        ...
        // Get indices of filtered elements (depth 0)
        int neighbors_idx[] = { image * GET_1D_INDEX(i * 2, j * 2, 0, 2 * width, depth),
    ...
    for (int i = 0; i < height / 2; ++i) {
      for (int j = 0; j < width / 2; ++j) {
        ...
        for (int k = 0; k < depth; ++k) {
          ...
          int max_neighbor_idx = *max_element(neighbors_idx, neighbors_idx + 4, [in, k](int a, int b) {
