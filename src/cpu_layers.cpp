        ...
        for (int f_i = 0; f_i < CONV_FILTER_SIZE; ++f_i) {
          ...
          for (int f_j = 0; f_j < CONV_FILTER_SIZE; ++f_j) {
            ...
            for (int d = 0; d < depth; ++d) {
              sum += in[GET_1D_INDEX(row, col, d, n, depth)] *
                     filter[f * GET_1D_INDEX(f_i, f_j, d, CONV_FILTER_SIZE, depth)];
            }
          }
        }

        out[GET_1D_INDEX(i, j, f, n, n_filter)] = sum;
      }
    }
  }
}
...
void cpu_max_pooling(float *in, float *out, int n, int depth) {
  for (int i = 0; i < n; i += 2) {
    for (int j = 0; j < n; j += 2) {
      // Get current pixel index (depth 0)
      int cur_idx = GET_1D_INDEX(i, j, 0, n / 2, depth);
    ...
    for (int j = 0; j < n; j += 2) {
      ...
      for (int k = 0; k < depth; ++k) {
        for (int neighbor_idx : neighbors_idx)
          out[neighbor_idx + k] = in[cur_idx + k];
      }
    }
  }
}
