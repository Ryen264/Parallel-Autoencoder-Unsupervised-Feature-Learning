...
static void parseAndNormalize(unsigned char* raw_data, float* images, int* labels, int num_samples, bool use_cuda) {
    ...
    if (use_cuda) {
        ...
        CUDA_CHECK(cudaMemcpy(d_raw_images, raw_images, image_data_size * sizeof(unsigned char), cudaMemcpyHostToDevice));
...
Dataset::Dataset(unique_ptr<float[]> &data, unique_ptr<int[]> &labels, int n, int width, int height, int depth):
