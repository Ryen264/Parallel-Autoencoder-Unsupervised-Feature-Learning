...
void Gpu_Autoencoder::_allocate_output_mem(int n, int width, int height) {
  ...
  CUDA_CHECK(cudaMalloc(&_out_encoder_bias_1,   n_pixel * ENCODER_FILTER_1_DEPTH * sizeof(float)));
  CUDA_CHECK(cudaMalloc(&_out_encoder_relu_1,   n_pixel * ENCODER_FILTER_1_DEPTH * sizeof(float)));
  CUDA_CHECK(cudaMalloc(&_out_avg_pooling_1,    n_pixel * ENCODER_FILTER_1_DEPTH * sizeof(float) / 4));

  CUDA_CHECK(cudaMalloc(&_out_encoder_filter_2, n_pixel * ENCODER_FILTER_2_DEPTH * sizeof(float) / 4));
  CUDA_CHECK(cudaMalloc(&_out_encoder_bias_2,   n_pixel * ENCODER_FILTER_2_DEPTH * sizeof(float) / 4));
  CUDA_CHECK(cudaMalloc(&_out_encoder_relu_2,   n_pixel * ENCODER_FILTER_2_DEPTH * sizeof(float) / 4));
  CUDA_CHECK(cudaMalloc(&_out_avg_pooling_2,    n_pixel * ENCODER_FILTER_2_DEPTH * sizeof(float) / 16));

  CUDA_CHECK(cudaMalloc(&_out_decoder_filter_1, n_pixel * DECODER_FILTER_1_DEPTH * sizeof(float) / 16));
  CUDA_CHECK(cudaMalloc(&_out_decoder_bias_1,   n_pixel * DECODER_FILTER_1_DEPTH * sizeof(float) / 16));
  CUDA_CHECK(cudaMalloc(&_out_decoder_relu_1,   n_pixel * DECODER_FILTER_1_DEPTH * sizeof(float) / 16));
  CUDA_CHECK(cudaMalloc(&_out_upsampling_1,     n_pixel * DECODER_FILTER_1_DEPTH * sizeof(float) / 4));

  CUDA_CHECK(cudaMalloc(&_out_decoder_filter_2, n_pixel * DECODER_FILTER_2_DEPTH * sizeof(float) / 4));
  CUDA_CHECK(cudaMalloc(&_out_decoder_bias_2,   n_pixel * DECODER_FILTER_2_DEPTH * sizeof(float) / 4));
  CUDA_CHECK(cudaMalloc(&_out_decoder_relu_2,   n_pixel * DECODER_FILTER_2_DEPTH * sizeof(float) / 4));
  CUDA_CHECK(cudaMalloc(&_out_upsampling_2,     n_pixel * DECODER_FILTER_2_DEPTH * sizeof(float)));

  CUDA_CHECK(cudaMalloc(&_out_decoder_filter_3, n_pixel * DECODER_FILTER_3_DEPTH  * sizeof(float)));
  CUDA_CHECK(cudaMalloc(&_out_decoder_bias_3,   n_pixel * DECODER_FILTER_3_DEPTH  * sizeof(float)));
...
void Gpu_Autoencoder::_forward_pass(float *data, int n, int width, int height, int depth) {
  int size = n * width * height * depth;
  CUDA_CHECK(cudaMemcpy(_batch_data, data, size * sizeof(float), cudaMemcpyHostToDevice));
  ...
  // ReLU layer
  gpu_relu(_out_encoder_bias_1, _out_encoder_relu_1, n, width, height, ENCODER_FILTER_1_DEPTH, _block_size_1D);

  // First avg pooling layer
  gpu_avg_pooling(_out_encoder_relu_1, _out_avg_pooling_1, n, width, height, ENCODER_FILTER_1_DEPTH, _block_size_3D_2);

  // Dim: n * w/2 * w/2 * 256
  // Second conv2D layer
  gpu_conv2D(_out_avg_pooling_1,
             _encoder_filter_2,
  ...
  // ReLU layer
  gpu_relu(_out_encoder_bias_2, _out_encoder_relu_2, n, width / 2, height / 2, ENCODER_FILTER_2_DEPTH, _block_size_1D);

  // Second avg pooling layer
  gpu_avg_pooling(_out_encoder_relu_2, _out_avg_pooling_2, n, width / 2, height / 2, ENCODER_FILTER_2_DEPTH, _block_size_3D_3);

  // First conv2D layer
  gpu_conv2D(_out_avg_pooling_2,
             _decoder_filter_1,
  ...
  // ReLU layer
  gpu_relu(_out_decoder_bias_1, _out_decoder_relu_1, n, width / 4, height / 4, DECODER_FILTER_1_DEPTH, _block_size_1D);

  // First upsampling layer
  gpu_upsampling(_out_decoder_relu_1, _out_upsampling_1, n, width / 4, height / 4, DECODER_FILTER_1_DEPTH, _block_size_3D_3);
  ...
  // ReLU layer
  gpu_relu(_out_decoder_bias_2, _out_decoder_relu_2, n, width / 2, height / 2, DECODER_FILTER_2_DEPTH, _block_size_1D);

  // Second upsampling layer
  gpu_upsampling(_out_decoder_relu_2, _out_upsampling_2, n, width / 2, height / 2, DECODER_FILTER_2_DEPTH, _block_size_3D_2);
  ...
  CUDA_CHECK(cudaMemcpy(_res_data, _out_decoder_bias_3, size * sizeof(float), cudaMemcpyDeviceToDevice));
}

float Gpu_Autoencoder::_fit_batch(float *data, int n, int width, int height, int depth, float learning_rate) {
  ...
  float loss = gpu_mse_loss(_batch_data,  //expected
                            _res_data,    //actual
                            n, width, height, depth, _block_size_1D);
  ...
  gpu_bias_grad(d_out,  //d_out
                d_in,   //d_bias
                n, width, height,
                DECODER_FILTER_3_DEPTH, _block_size_1D);

  gpu_update_weight(_decoder_bias_3,  //weight
                    d_in,             //gradient
                    DECODER_FILTER_3_DEPTH, learning_rate, _block_size_1D);
  ...
  gpu_conv2D_grad(_out_upsampling_2,  //in
                  ...
                  d_filter,           //d_filter
                  n, width, height,
                  DECODER_FILTER_2_DEPTH, DECODER_FILTER_3_DEPTH, _block_size_3D_1);
  ...
  gpu_update_weight(_decoder_filter_3,  //weight
                    d_filter,           //gradient
                    DECODER_FILTER_3_SIZE, learning_rate, _block_size_1D);

  // Pass through upsampling (dim: n * w/2 * w/2 * 256)
  gpu_upsampling_backward(d_out,  //d_out
                          d_in,   //d_in
                          n, width / 2, height / 2,
                          DECODER_FILTER_2_DEPTH, _block_size_3D_2);
  ...
  gpu_relu_backward(_out_decoder_bias_2,  //in
                    ...
                    d_out,                //d_out
                    n, width / 2, height / 2,
                    DECODER_FILTER_2_DEPTH, _block_size_1D);
  ...
  gpu_update_weight(_decoder_bias_2,  //weight
                    d_in,             //gradient
                    DECODER_FILTER_2_DEPTH, learning_rate, _block_size_1D);

  gpu_conv2D_grad(_out_upsampling_1,  //in
                  d_out,              //d_out
                  d_filter,           //d_filter
                  n, width / 2, height / 2,
                  DECODER_FILTER_1_DEPTH, DECODER_FILTER_2_DEPTH, _block_size_3D_2);
  ...
  gpu_update_weight(_decoder_filter_2,  //weight
                    d_filter,           //gradient
                    DECODER_FILTER_2_SIZE, learning_rate, _block_size_1D);

  // Upsampling (dim: n * w/4 * w/4 * 128)
  gpu_upsampling_backward(d_out,  //d_in
                          d_in,   //d_out
                          n, width / 4, height / 4,
                          DECODER_FILTER_1_DEPTH, _block_size_3D_3);
  ...
  gpu_relu_backward(_out_decoder_bias_1,  //in
                    ...
                    d_out,                //d_out
                    n, width / 4, height / 4,
                    DECODER_FILTER_1_DEPTH, _block_size_1D);
  ...
  gpu_update_weight(_decoder_bias_1,  //weight
                    d_in,             //gradient
                    DECODER_FILTER_1_DEPTH, learning_rate, _block_size_1D);

  gpu_conv2D_grad(_out_avg_pooling_2,  //in
                  d_out,               //d_out
                  d_filter,            //d_filter
                  n, width / 4, height / 4,
                  ENCODER_FILTER_2_DEPTH, DECODER_FILTER_1_DEPTH, _block_size_3D_3);
  ...
  gpu_update_weight(_decoder_filter_1,  //weight
                    d_filter,           //gradient
                    DECODER_FILTER_1_SIZE, learning_rate, _block_size_1D);

  // Max pooling backwards (dim: n * w/2 * w/2 * 128)
  gpu_avg_pooling_backward(d_out,  //d_out
                           d_in,   //d_in
                           n, width / 2, height / 2,
                           ENCODER_FILTER_2_DEPTH, _block_size_3D_2);

  gpu_relu_backward(_out_encoder_bias_2,  //in
                    d_in,                 //d_in
                    d_out,                //d_out
                    n, width / 2, height / 2,
                    ENCODER_FILTER_2_DEPTH, _block_size_1D);
  ...
  gpu_update_weight(_encoder_bias_2,  //weight
                    d_in,             //gradient
                    ENCODER_FILTER_2_DEPTH, learning_rate, _block_size_1D);

  gpu_conv2D_grad(_out_avg_pooling_1,  //in
                  d_out,               //d_out
                  d_filter,            //d_filter
                  n, width / 2, height / 2,
                  ENCODER_FILTER_1_DEPTH, ENCODER_FILTER_2_DEPTH, _block_size_3D_2);
  ...
  gpu_update_weight(_encoder_filter_2,  //weight
                    d_filter,           //gradient
                    ENCODER_FILTER_2_SIZE, learning_rate, _block_size_1D);

  gpu_avg_pooling_backward(d_out,   //d_out
                           d_in,    //d_in
                           n, width, height,
                           ENCODER_FILTER_1_DEPTH, _block_size_3D_1);
  ...
  gpu_bias_grad(d_out,
                d_in,
                n, width, height,
                ENCODER_FILTER_1_DEPTH, _block_size_1D);

  gpu_update_weight(_encoder_bias_1,
                    d_in,
                    ENCODER_FILTER_1_DEPTH, learning_rate, _block_size_1D);
  ...
  gpu_update_weight(_encoder_filter_1,
                    d_filter,
                    ENCODER_FILTER_1_SIZE, learning_rate, _block_size_1D);
  ...
  for (int epoch = 1; epoch <= n_epoch; ++epoch) {
    ...
    for (int i = 0; i < n_batch; ++i) {
      ...
      float batch_loss = _fit_batch(dataset.get_data() + offset * image_size,
                                    cur_batch_size, width, height, depth, learning_rate);
      timer.stop();
...
Dataset Gpu_Autoencoder::encode(const Dataset &dataset) const {
  ...
  float *a, *b;
  CUDA_CHECK(cudaMalloc(&a, ENCODE_BATCH_SIZE * width * height * MAX_FILTER_DEPTH * sizeof(float)));
  CUDA_CHECK(cudaMalloc(&b, ENCODE_BATCH_SIZE * width * height * MAX_FILTER_DEPTH * sizeof(float)));

  for (int i = 0; i < n_batch; ++i) {
    int in_offset = i * ENCODE_BATCH_SIZE;
    int cur_batch_size = min(ENCODE_BATCH_SIZE, n - in_offset);

    CUDA_CHECK(cudaMemcpy(b, dataset.get_data() + in_offset * image_size, cur_batch_size * image_size * sizeof(float), cudaMemcpyHostToDevice));
    ...
    // ReLU
    gpu_relu(b, a, cur_batch_size, width, height, ENCODER_FILTER_1_DEPTH, _block_size_1D);

    // Max pooling
    gpu_avg_pooling(a, b, cur_batch_size, width, height, ENCODER_FILTER_1_DEPTH, _block_size_3D_2);
    ...
    // Second ReLU
    gpu_relu(b, a, cur_batch_size, width / 2, height / 2, ENCODER_FILTER_2_DEPTH, _block_size_1D);

    // Second max pooling
    gpu_avg_pooling(a, b, cur_batch_size, width / 2, height / 2, ENCODER_FILTER_2_DEPTH, _block_size_3D_3);

    // Copy batch
    CUDA_CHECK(cudaMemcpy(res.get_data() + out_offset * encoded_image_size, b, cur_batch_size * encoded_image_size * sizeof(float), cudaMemcpyDeviceToHost));
...
Dataset Gpu_Autoencoder::decode(const Dataset &dataset) const {
  ...
  for (int i = 0; i < n_batch; ++i) {
    ...
    // ReLU
    gpu_relu(b, a, cur_batch_size, width, height, DECODER_FILTER_1_DEPTH, _block_size_1D);

    // Upsampling
    gpu_upsampling(a, b, cur_batch_size, width, height, DECODER_FILTER_1_DEPTH, _block_size_3D_3);
    ...
    // Second ReLU
    gpu_relu(b, a, cur_batch_size, width * 2, height * 2, DECODER_FILTER_2_DEPTH, _block_size_1D);

    // Second upsampling
    gpu_upsampling(a,b, cur_batch_size, width * 2, height * 2, DECODER_FILTER_2_DEPTH, _block_size_3D_2);
...
float Gpu_Autoencoder::eval(const Dataset &dataset) const {
  ...
  CUDA_CHECK(cudaMemcpy(expected, dataset.get_data(), size * sizeof(float), cudaMemcpyHostToDevice));
  CUDA_CHECK(cudaMemcpy(actual,   res.get_data(),     size * sizeof(float), cudaMemcpyHostToDevice));
