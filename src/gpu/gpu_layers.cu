...
// -------------------- Upsampling (2x up) --------------------
__global__ void
gpu_upsampling(float *in, float *out, int width, int height, int depth) {
  int x          = blockIdx.x * blockDim.x + threadIdx.x;
  int y          = blockIdx.y * blockDim.y + threadIdx.y;
  int d          = blockIdx.z * blockDim.z + threadIdx.z;
  int new_width  = width * 2;
...
__global__ void gpu_max_pooling_backward(
  ...
  float *out = d_in + GET_1D_IDX(y, x, d, width, depth);
  float  val = d_out[GET_1D_IDX(out_y, out_x, d, width / 2, depth)];

  int neighbors_idx[] = { GET_1D_INDEX(out_y * 2, out_x * 2, 0, width, depth),
                          GET_1D_INDEX(out_y * 2, out_x * 2 + 1, 0, width, depth),
                          GET_1D_INDEX(out_y * 2 + 1, out_x * 2, 0, width, depth),
                          GET_1D_INDEX(out_y * 2 + 1, out_x * 2 + 1, 0, width, depth) };
  int max_neighbor_idx =
      *max_element(neighbors_idx, neighbors_idx + 4, [in, k](int a, int b) {
        return in[a + k] < in[b + k];
      });

  *out = (y * width + x == max_neighbor_idx) ? val : 0.0f;
}