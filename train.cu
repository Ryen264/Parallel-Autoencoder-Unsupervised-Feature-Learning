...
struct Conv2D {
    ...
    void load(const string &prefix){
        ...
        if(f){
            size_t _read_w = fread(weight.data(), sizeof(float32), weight.size(), f);
            (void)_read_w;
            fclose(f);
        }
        string fn2 = prefix + "_b.bin";
        ...
        if(f){
            size_t _read_b = fread(bias.data(), sizeof(float32), bias.size(), f);
            (void)_read_b;
            fclose(f);
        }
    }
...
struct Autoencoder {
    ...
    // separate instances for stateful layers (one per layer)
    ...
    Tensor4D forward(const Tensor4D &x){
        // encoder
        a1 = conv1.forward(x);
        Tensor4D r1 = relu1.forward(a1);
        a1p = pool1.forward(r1);

        a2 = conv2.forward(a1p);
        Tensor4D r2 = relu2.forward(a2);
        a2p = pool2.forward(r2);

        z = conv3.forward(a2p);
        Tensor4D rz = relu3.forward(z); // latent activation

        // decoder
        Tensor4D up_1 = up1.forward(rz); // 8->16
        Tensor4D d1 = deconv1.forward(up_1);
        Tensor4D rd1 = relu4.forward(d1);

        Tensor4D up_2 = up2.forward(rd1); // 16->32
        Tensor4D d2 = deconv2.forward(up_2);
        Tensor4D rd2 = relu5.forward(d2);
    ...
    float32 compute_loss_and_backward(const Tensor4D &input, float32 lr){
        ...
        // decoder backward (mirror of forward, using matching instances)
        Tensor4D g_deconv3 = deconv3.backward(grad, lr); // returns grad wrt rd2
        Tensor4D g_rd2 = relu5.backward(g_deconv3);
        Tensor4D g_deconv2 = deconv2.backward(g_rd2, lr);
        Tensor4D g_up2 = up2.backward(g_deconv2);
        Tensor4D g_rd1 = relu4.backward(g_up2);
        Tensor4D g_deconv1 = deconv1.backward(g_rd1, lr);
        Tensor4D g_up1 = up1.backward(g_deconv1);
        Tensor4D g_rz = relu3.backward(g_up1);
        // encoder backward
        Tensor4D g_conv3 = conv3.backward(g_rz, lr);
        Tensor4D g_pool2 = pool2.backward(g_conv3);
        Tensor4D g_r2_back = relu2.backward(g_pool2);
        Tensor4D g_conv2 = conv2.backward(g_r2_back, lr);
        Tensor4D g_pool1 = pool1.backward(g_conv2);
        Tensor4D g_r1_back = relu1.backward(g_pool1);
    ...
    Tensor4D get_latent(const Tensor4D &x) {
    // encoder only
    a1 = conv1.forward(x);
    Tensor4D r1 = relu1.forward(a1);
    a1p = pool1.forward(r1);
    ...
    z = conv3.forward(a2p);
    Tensor4D rz = relu3.forward(z); // latent activation
    return rz; // trả về latent feature
}
};
